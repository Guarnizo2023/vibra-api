...
export class WeeklyScheduler {
    ...
    async generateWeeklyPlan() {
        ...
        try {
            ...
            const activities: Activity[] = await this.activityService.assignActivities(emotions);
            console.log('activities:', activities);

            const weekSchedule = activities.map((activity: Activity, index) => ({
                date: this.getNextWeekDate(index),
                emotion: activity.emotion._id,
            ...
            await this.activityService.createWeeklySchedule({
                ...
                days: weekSchedule as { date: Date; emotion: string; activity: string; status: "pending" | "completed" | "skipped"; }[],
