...
export class RankingService implements OnGatewayConnection {
    ...
    private async calculateDailyRankings() {
        ...
        const sortedRankings = Array.from(userScores.entries())
            .map(([userId, score]) => ({
                userId,
                score,
                username: responses.find(r => r.user._id.toString() === userId)?.user["username"],
                avatar: responses.find(r => r.user._id.toString() === userId)?.user["avatar"]
            }))
    ...
    private calculateScore(response: UserResponse): number {
        const baseScore = response.responses.reduce((acc, curr) =>
            acc + (curr.isCorrect ? 100 : 0), 0);
